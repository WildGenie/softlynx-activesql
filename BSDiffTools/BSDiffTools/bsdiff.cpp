...
void  err(int a, wchar_t * format, wchar_t * param)
{
	wchar_t buf[1024];
...
void err(int a, wchar_t * message)
{
	err(a,L"%s",message);
}
...
int bsdiff(wchar_t *argv[])
{
	...
	/* Allocate newsize+1 bytes instead of newsize bytes to ensure
		that we never try to malloc(0) and get a NULL pointer */
	if(((fd=_wfopen(argv[2],L"rb"))<0) ||
		(fseek(fd,0,SEEK_END)!=0) ||
		((newsize=ftell(fd)) == 0) ||
		((pnew=(u_char*)malloc(newsize+1))==NULL) ||
		(fseek(fd,0,SEEK_SET)!=0) ||
		(fread(pnew,1, newsize, fd)!=newsize) ||
		(fclose(fd)==-1)) err(1,L"Can't handle %s",argv[2]);
	...
	if((fd=_wfopen(argv[3],L"wb"))<0)
		err(1,L"Can't handle %s",argv[3]);
	...
	if(fwrite(header,1,32,fd)!=32) err(1,L"Can't handle %s",argv[3]);
	...
	bz = BZ2_bzWriteOpen ( &bzerror, fd, 
                         blockSize100k, 0, 30);
    if (bzerror != BZ_OK) err(1, L"Problem bzWriteOpen");
	...
	while(scan<newsize) {
		...
		if((len!=oldscore) || (scan==newsize)) {
			...
			offtout(lenf,buf);
			if((8 != BZ2_bzwrite(bz,buf,8)) || (bzerror != BZ_OK)) err(1,L"bzwrite");
	...
	if((fseek(fd,0,SEEK_END))!=0 || ((len=ftell(fd))==-1)) err(1,L"problem 0: %s",argv[3]);
	offtout(len-32,buf);
	if((fseek(fd,8,SEEK_SET)!=0) || (ftell(fd) != 8) || (fwrite(buf,1,8,fd)!=8))
		err(1,L"problem 1: %s",argv[3]);
	offtout(newsize,buf);
	if((fseek(fd,24,SEEK_SET)!=0) || (ftell(fd) != 24)|| (fwrite(buf,1,8,fd)!=8))
		err(1,L"problem 2:%s",argv[3]);

	if(fseek(fd,0,SEEK_END)!=0) err(1,L"problem 3: %s",argv[3]);
	...
	if((fseek(fd,0,SEEK_END)!=0) || ((newsize=ftell(fd))==-1)) err(1,L"problem 4: %s",argv[3]);
	offtout(newsize-len,buf);
	if((fseek(fd,16,SEEK_SET)!=0) || (ftell(fd)!=16) || (fwrite(buf,1,8,fd)!=8))
		err(1,L"problem 5:%s",argv[3]);

	if(fseek(fd,0,SEEK_END)!=0 || (ftell(fd) == -1)) err(1,L"problem 6: %s",argv[3]);

	bz = BZ2_bzWriteOpen ( &bzerror, fd, 
                         blockSize100k, 0, 30);
    if (bzerror != BZ_OK) err(1, L"Problem bzWriteOpen");
	if((eblen != BZ2_bzwrite(bz,eb,eblen)) || (bzerror != BZ_OK)) err(1,L"bzwrite");
	BZ2_bzWriteClose ( &bzerror, bz, 0, NULL, NULL);
 	fclose(fd);
	free(db);
	free(eb);
